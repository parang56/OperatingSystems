...
void increment_cpu_wait_time(struct proc *current_proc) {
    // Loop through all queues and increment cpu_wait for runnable processes, excluding the current process
    for (int level = 0; level < NQUEUE; level++) {
        for (int i = 0; i < NPROC; i++) {
            struct proc *p = queue[level][i];
...
// Initialize the process table and MLFQ queues
void pinit(void) {
...
// Otherwise return 0.
// Allocate a new process and initialize MLFQ fields
...
// Create a new process by copying the current process
int fork(void) {
  int i, pid;
  struct proc *np;
  struct proc *curproc = myproc();
  ...
  *np->tf = *curproc->tf;
  ...
  #ifdef ANALYZE
  if(np->pid > 3 && strncmp(np->name, "sh", 2))
  { 
    cprintf("PID %d forked at tick %d\n", np->pid, np->entry_time);
  }
  #endif
...
exit(void)
{
  ...
  #ifdef ANALYZE
  if(curproc->pid > 2 && strncmp(curproc->name, "sh", 2))
  {
    cprintf("PID %d terminated at tick %d\n", curproc->pid, ticks);
  }
  #endif
...
void aging_processes(void) {
    ...
    for (int level = 1; level < NQUEUE; level++) {
        for (int i = 0; i < NPROC; i++) {
            struct proc *p = queue[level][i];
            // If process is runnable and if waiting time is bigger than aiging t.h.
  ...
  for(int i = 0; i < NPROC - 1; i++)
  {
    for(int j = 0; j < NPROC - i - 1; j++)
    {
      ...
      if(p1 != 0 && p2 != 0)
      {
        ...
        else if (p1->io_wait_time == p2->io_wait_time && p1->queue_entry_time < p2->queue_entry_time) {
          struct proc *temp = queue[level][j];
          queue[level][j] = queue[level][j + 1];
          queue[level][j + 1] = temp;
        }
        // If io_wait_time is the same, and entry time is same, later pid gets prio.
        else if (p1->io_wait_time == p2->io_wait_time && p1->queue_entry_time == p2->queue_entry_time && p1->pid < p2->pid) {
          struct proc *temp = queue[level][j];
          queue[level][j] = queue[level][j + 1];
          queue[level][j + 1] = temp;
        }
    ...
    for(int level = 0; level < NQUEUE; level++)
    {
      ...
      for(int i = 0; i < NPROC; i++)
      {
        ...
        {    
          ...
          c->proc = p;
          switchuvm(p); // Set process memory space
          p->state = RUNNING;
